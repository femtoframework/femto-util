 ...
 * should invokeMethod the the timer's <tt>cancel</tt> method.
 * <p/>
...
//            delay = 0;
    ...
    private void sched(TimerTask task, long time, long period)
    {
        ...
        synchronized (queue) {
            ...
            synchronized (task) {
                if (task.getState() != TimerState.VIRGIN) {
                    throw new IllegalStateException("Task already scheduled or cancelled");
                }
                task.setNextExecutionTime(time);
                task.setPeriod(period);
                task.setState(TimerState.SCHEDULED);
            }
        ...
        while (true) {
            try {
                ...
                synchronized (queue) {
                    ...
                    task = queue.getMin();
                    synchronized (task) {
                        if (task.getState() == TimerState.CANCELLED) {
                            queue.removeMin();
                        ...
                        if (taskFired = (executionTime <= currentTime)) {
                            long n = task.nextExecutionTime(currentTime);
                            if (n == -1) {
                                queue.removeMin();
                                task.setState(TimerState.EXECUTED);
                            }
                            else { // Repeating task, reschedule
                                queue.rescheduleMin(n);
                            }
                        }
                    }
...
class TaskQueue
{
    ...
    private void fixDown(int k)
    {
        ...
        while ((j = k << 1) <= size) {
            if (j < size &&
                queue[j].getNextExecutionTime() > queue[j + 1].getNextExecutionTime()) {
